...
function PosModal({ item, warehouses, onClose, onSaved }: { item: PosLocation | null; warehouses: Warehouse[]; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      const { error } = item ? await supabase.from('pos_locations').update(payload).eq('id', item.id) : await supabase.from('pos_locations').insert({ ...payload, company_id: '00000000-0000-0000-0000-000000000001' });
